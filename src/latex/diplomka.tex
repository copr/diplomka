...
Ve statistice jsou nejrozšířenější dva přístupy a to frekventistický a Bayesovký. Rozdíl mezi nimi je v tom, jak se staví k parametrům daného statistické modelu. Ve statistice frekventistické považujeme tyto parametry za nějaké konkrétní, ale neznáme hodnoty. Zatímco ve statistice Bayesovské považujeme tyto parametry za náhodné veličiny, které mají pravděpodobnostní rozdělení, jemuž říkáme apriorní \cite{robert2007bayesian}. Pomocí apriorního rozdělení můžeme do modelu zanést naši předchozí zkušenost nebo informaci, která není obsažena v samotných pozorováních. Toto rozdělení budeme značit 
\begin{align}
f(\theta), \theta \in \mathcal{R}^m.
\end{align}\par
Dalším předpoklad je, že daná pozorování jsou z parametrizovaného pravděpodobnostního rozdělení s hustotou pravděpodobnosti 
\begin{align}
f(x | \theta),
\end{align}
kde $x$ je vektor pozorování $(x_0, x_1, ..., x_n)$ a $\theta$ je vektor parametrů $(\theta_0, \theta_1, ... \theta_m)$. \par
Nyní můžeme pomocí Bayesovy věty odvodit podmíněné pravděpodobnostní rozdělení parametru $\theta$ 
\begin{align} \label{bayes}
f(\theta | x) = \frac{f(x|\theta)f(\theta)}{f(x)},
\end{align}
toto rozdělení nazýváme aposteriorní a shrnuje veškerou informaci o parametru $\theta$, která je nám známa. $f(x)$ lze chápat jako normalizační konstantu, která nezávisí na $\theta$ a je možné ji získat vyintegrováním čitatele v \ref{bayes}  přes $\theta$
\begin{align}
f(x) = \int f(x|\theta)f(\theta) d\theta.
\end{align}

\subsection{Bayesovská volba modelu}
Nyní předpokládejme, že máme množinu $K$ modelů $M = \{M_1, ..., M_K\}$. Pro každý z nich předpokládáme, že data mají pravděpodobnostní rozdělení $f(x | \theta_k, M_k)$, kde $\theta_k$ určuje parametry modelu $M_k$. Nyní bychom chtěli rozhodnout, z kterého modelu pocházejí data nebo jinak který model nejlíp vysvětluje data. Z Bayesovkého pohledu budeme tedy chtít odvodit aposteriorní rozdělení na modelu, proto potřebujeme apriorní rozdělení $f(\theta_k, M_k)$, které můžeme přirozeně faktorizovat jako 
\begin{align}
f(\theta_k, M_k) = f(\theta_k | M_k)f(M_k).
\end{align}
