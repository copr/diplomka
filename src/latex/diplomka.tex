...
Ve statistice jsou nejrozšířenější dva přístupy a to frekventistický a Bayesovký. Rozdíl mezi nimi je v tom, jak se staví k parametrům daného statistického modelu. Ve statistice frekventistické považujeme tyto parametry za nějaké konkrétní, ale neznáme hodnoty. Zatímco ve statistice Bayesovské považujeme tyto parametry za náhodné veličiny. Rozdělení těchto náhodných veličin nazýváme apriorní \cite{robert2007bayesian}. Pomocí apriorního rozdělení můžeme do modelu zanést naši předchozí zkušenost nebo informaci, která není obsažena v samotných pozorováních. Nyní zadefinujeme náhodné veličiny a jejich pravděpodobnostní hustoty:
\begin{itemize}
	\item $X$ ... data
	\item $\Theta$ ... neznámý parametr
	\item $f_{\Theta}(\theta)$ ... apriorní rozdělení parametru $\Theta$ 
	\item $f_{X|\Theta}(x|\theta)$ ... model - z bayesovského pohledu podmíněná hustota
\end{itemize}
$X$ je vektor pozorování $( X_1, ..., X_n)$ a $\Theta$ je vektor parametrů $(\Theta_1, ... \Theta_m)$. \par
Nyní můžeme pomocí Bayesovy věty odvodit podmíněnou pravděpodobnostní hustotu parametru $\theta$ 
\begin{align} \label{bayes}
f(\theta | x)_{\Theta|X} = \frac{f_{X|\Theta}(x|\theta)f_{\Theta}(\theta)}{f(x)},
\end{align}
toto rozdělení nazýváme aposteriorní a shrnuje veškerou informaci o parametru $\theta$, která je nám známa, tj. apriorní informace a informace z dat. $f(x)$ v \eqref{bayes} je marginální hustota ze sdružené hustoty $f_{X|\Theta}(x|\theta)f_{\Theta}(\theta)$, tudíž ji lze chápat jako normalizační konstantu, která nezávisí na $\theta$ a je možné ji získat vyintegrováním čitatele v \eqref{bayes}  přes $\theta$
\begin{align}
f(x) = \int_{\Theta} f(x|\theta)f(\theta) d\theta.
\end{align}
V dalším textu, pokud bude význam zřejmý z kontextu
\begin{itemize}
	\item nebudeme rozlišovat náhodné veličiny a jejich hodnoty, 
	\item funkce nebudeme rozlišovat indexy, ale jen argumenty,
	\item všechny integrály budeme uvažovat určité pro obor hodnot.
\end{itemize}
\subsection{Bayesovská volba modelu}
V předchozí části jsme se zabývali situací s jedním konkrétním modelem, nyní se zabývejme obecnější situací, kdy máme množinu více modelů. Předpokládejme tedy, že máme $K$ modelů a náhodnou veličinu $M$, která nabývá hodnot $\{1, ..., K\}$, tato náhodná veličina určuje model. Pro každý z nich předpokládáme, že data mají pravděpodobnostní hustotu $f(x |m,  \theta_m)$, kde $\theta_k$ určuje parametry modelu $m$. Množina parametrů modelu $f(x |m,  \theta_m)$ je:
\begin{align}
\boldsymbol{\Theta} = \bigcup_{m=1}^{K} {m} \times \Theta_m, kde \Theta_m \subset \mathcal{R}^{D_m} je množina hodnot parametrů modelu m.
\end{align}

Apriorní rozdělení budeme volit takto:
\begin{align}
f(m, \theta) =   f(m)\prod_{\tilde{m}}f_{\Theta_m|M}(\theta_{\tilde{m}}| M=\tilde{m}) = f(m)f(\theta_m | m)
\end{align}
tj. předpokládáme podmíněnou nezávislost na parametrech $\theta_1, ... \theta_{m-1}, \theta_{m+1}, \theta_{K}$ za podmínky $M=m$.
Pro model bude obdobně platit
\begin{align}
f(x | m, \theta) = f(x | m, \theta_m).
\end{align}
Aposteriorní rozdělení tedy bude vypadat následovně:
\begin{align}
f(\theta_k, M_k | x) = 
\frac{f(x | \theta_k, M_k)f(\theta_k | M_k)f(M_k)}{\sum_k \int f(x | \theta_k, M_k)f(\theta_k | M_k)f(M_k) d\theta_k}.
\end{align}
A vyintegrováním $\theta_k$ dostáváme pravděpodobnostní rozdělení, které nám udává pravděpodobnost, že data pocházejí z modelu $M_k$:
\begin{align}
f(M_k | x) =  \frac{\int f(x | \theta_k, M_k)f(\theta_k | M_k) d\theta_k f(M_k)}{\sum_k \int f(x | \theta_k, M_k)f(\theta_k | M_k)f(M_k) d\theta_k}.
\end{align}  
...
%Lineární regrese se používá k vysvětlení souboru dat pomocí přímky, tedy používá se k nalezení parametrů přimky. Je několik přístupů, jimiž lze nálezt paremtry přímky, a to buď metodou nejmenších čtverců, maximum likelihood estimatorem \myworries{spravny preklad} a nebo bayesovsky. My budeme využívat bayesovká přistup a to z toho důvodu, že ostatní přístupy by nám nestačily v případě, kdy nevíme kolik je parametrů v modelu.
