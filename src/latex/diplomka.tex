...
\end{align}
\section{MCMC}
Aposteriorní distribuce mají často takový tvar, že je jakýkoli analytický výpočet nemožný. V takový moment se buď musíme spokojit s jednodušším modelem, jehož aposteriorní distribuce umožňuje analytický výpočet a nebo se musíme uchýlit k numerickým aproximacím dané distribuce. Toto nám umožňují algoritmy Monte Carlo, které dané distribuce aproximují pomocí množiny vzorků. Samotné algoritmy Markov Chain Monte Carlo (MCMC) tvoří podmnožinu algoritmů MC. Z vzorků získaných MC algoritmem můžeme například aproximovat střední hodnotu náhodné veličiny (tj. výpočet integrálu), získat přibližný odhad spolehlivosti či přibližně vyřešit optimalizační problém. \par
...
Již jsme naznačili,že  myšlenkou MC simulací je aproximace distribuce $f(x)$, definované na prostoru $\mathbb{X}$, množinou nezávislých vzorků $\{x^{(i)}\}_{i=1}^{N}$. Je tedy možné aproximovat integrály $I(h)$ posloupností sum $I_n(h)$, která konverguje následujícím způsobem
\begin{align}
	I_N(h) = \frac{1}{N}\sum_{i=1}^N h(x^{(i)}) \xrightarrow{s.j.} I(f) = \int_{\mathbb{X}} h(x)f(x)dx.
\end{align}
Tedy, odhad $I_N(h)$ je nestranný odhad a podle zákona velkých čísel bude konvergovat skoro jistě k $I(h)$, tedy ... . Pokud rozptyl $h(x)$ splňuje $\sigma^2_h = E_{f(x)}(f^2(x) - I^2(x)) < \infty$, pak je $var(I_N(h)) = \frac{\sigma^2_h}{H}$ a podle centrální limitní věty bude platit
\begin{align}
	\sqrt{N}(I_N(h) - I(h)) \Rightarrow \mathcal{N}(0, \sigma^2_f),
\end{align}
kde $\Rightarrow$ značí konvergenci v distribuci, tj. pokud platí $\lim_{n\rightarrow \infty} F_n(x) = F(x)$ pro všechna  $x \in \mathbb{R}$, kde je $F$ spojitá. Zde je $F_n$ respektive $F$  distribuční funkce náhodné veličiny $X_n$ respektive $X$.
\par
...
%\end{align}
Chceme-li počítat integrál, kde nejde primárně o výpočet střední hodnoty, je možné jej jako výpočet střední hodnoty vyjádřit. Proto je potřeba zvolit distribuci $f(x)$ z které budeme vygenerovat vzorky. Pro výpočty integrálů s omezeným integračním oborem se jeví výhodné použít uniformní distribuci. Toto nemusí být především ve vyšších dimenzích vhodné, protože vedou k vysokému rozptylu odhadu a tedy k nutnosti použít vysoký počet vzorků. Další možnosti řešení jsou použiti metod Rejection sampling, Importance sampling \cite{andrieu2003introduction} a nebo v případě kdy už nejsme schopni vzorkovat pomocí předchozích metod použít jeden z MCMC algoritmů. \par
\subsection{Markovský řetězec}
Markovův řetězec je posloupnost náhodných veličin, s pravděpodobností přechodu závisející na konkretním stavu ve kterém se řetězec nachází. \par
V celé této části budeme uvažovat pravděpodobnostní prostor $(\mathcal{X}, \mathcal{B}(\mathcal{X}), P)$, kde $\mathcal{X} = \mathbb{R}^n$, $\mathcal{B}$ je borelovská $\sigma$-algebra a $P$ je pravděpodobnostní míra.
\begin{definition}
...
\begin{definition}
	Mějme přechodové jádro K, posloupnost $X_1, ..., X_n, ...$ náhodných veličin je Markovský řetězec s přechodovým jádrem $K$, pokud pro $t \in \mathbb{N}$ podmíněna distribuce $X_{t+1}$ za $X_t = x_{t}, X_{t-1}=x_{t-1}, ..., X_1 = x_1$ je stejná jako distribuce $X_{t+1}$ za $X_t = x_t$, tedy:
	\begin{equation*}
		P(X_{t+1} \in A| X_t = x_{t}, X_{t-1}=x_{t-1}, ..., X_1 = x_1) = P(X_{t+1} \in A| X_t = x_t) \\
		= \int_A K(x_t, dx).
	\end{equation*}
	Řekneme, že řetězec je homogenní pokud:
	\begin{equation*}
		P(X_{t+1} | y) = P(X_t | y).
	\end{equation*}
\end{definition} \par
Když je $\mathcal{X}$ diskrétní, přechodové jádro lze charakterizovat přechodovou maticí K s prvky
\begin{equation*}
P_{xy} = P(X_t = y| X_{t-1} = x), x, y \in (\mathcal{X}).
\end{equation*}

Ve spojitém případě jádro také určuje podmíněnou hustotu $\mathcal{K}(x, x^{,}):\mathbb{R}^n \times \mathbb{R}^n \rightarrow \mathbb{R}$ přechodu $K(x, \cdot)$, tedy $P(X \in A| x) = K(x, A) = \int_A \mathcal{K}(x, x^{,}) dx^{,}$. 
...
\begin{definition}
	...
	neredukovatelný pokud, pro všechny $A \in \mathcal{B}(\mathcal{X})$ s $\phi(A) > 0$, existuje n takové, že $K^n(x, A) > 0$ pro všechna $x \in \mathcal{X}$, kde $K^n$ je n-té složení $K$, tedy $K^n = K \circ K^{n-1}$.  \\
\end{definition}
...
\end{align}
kde $\gcd$ znamená největší společný dělitel. Říkáme tedy, že neredukovatelný Markovský řetěz je aperiodický pokud má periodu 1. Diskuzi obecného případu necháváme na vhodném zdroji \cite{robert2004monte}. \par
...
\begin{align}
K_{MH}(x^i, A) = \int_A q(x | x^i) A(x^i, x)dx + \delta_{x^i}(A)r(x^{i}) 
\end{align}
...
Důležitou vlastností MCMC algoritmů je možnost kombinovat několik přechodových jader pro konstrukci jednoho Markovova řetězce. Pokud tedy máme dvě přechodová jádra $K_1$, $K_2$ a $\pi(x)$ je invariantní distribuce pro obě jádra, pak hybridní jádro $vK_1 + (1-v)K_2$, kde $v \in [0,1]$, je také přechodové jádro s invariantní distribucí $\pi(x)$. Dále taky můžeme tyto dvě jádra použít v cyklu po sobě, tedy nejdřív navrhnout nový vzorek pomocí $K_1$ a poté navrhnout další vzorek pomocí $K_2$ a výsledné přechodové jádro bude mít opět invariantní distribuci $\pi(x)$ \cite{andrieu2003introduction}. \par
Míchání jader můžeme využít například v případech, kdy $\pi(x)$ má několik extrému, ale mezi nimi oblasti s malou pravděpodobností. V takovém případě může být výhodné mít jedno jádro, které bude navrhovat větší skoky a tím nám umožní postupně procházet všechny extrémy a poté mít druhé jádro, které bude navrhovat malé skoky, čímž prozkoumáme $\pi(x)$ detailněji.  \par
Cyklus MCMC jader se často využívá, když chceme pro různé části parametru použít jiné návrhové distribuce nebo v případě kdy nový vzorek přijímáme postupně, tak jak je  znázorněno v algoritmu \ref{src:super_verze}.
...
Nyní si ukážeme algoritmus Reversible Jump Markov Chain Monte Carlo (RJMCMC)\cite{green2009reversible}, jenž je zobecněním Metropolisova-Hastingsova algoritmu a dokáže vzorkovat z aposteriorní distribuce s parametry různé dimenze. S takovou situací jsme se teoreticky seznámili v podkapitole Bayesovská volba modelu \ref{model_choice}. 
 \par
Půjde nám, podobně jako v předchozí kapitole, o to, sestavit markovovský řetězec tak, aby jeho stacionární distribuce byla $\pi(x)$, kde $x = (m, \theta_m)$. V konstrukci tohoto řetězce je třeba kombinovat dva typy přechodových jader a to takové, které umožňují procházení prostoru $\boldsymbol{\Theta_m}$  pro  model $m$ a takové které dokáží přejít z jednoho modelu do druhého, tj. přechod z $x=(m, \theta_m)$ do $x^{,} = (m^{,}, \theta_{m^{,}})$, kde $m^{,}$ je index modelu do kterého přecházíme. V prvním případě se používají klasické MCMC algoritmy, tak jak byly popsány v předchozí kapitole. \par
Různé modely ale mohou mít různou dimenzi a toto přináší s sebou problém s výpočtem pravděpodobnosti přijetí, protože hustoty, v modelu z kterého vycházíme, mohou být vzhledem k jiné míře než hustoty modelu do kterého přecházíme. Zmíněné míry jsou zpravidla Lebesgueovy míry, ale na prostorech jiné dimenze, RJMCMC tento problém řeší tím, že dorovná dimenze použitím náhodných veličin. Tímto se zajistí, že hustoty jsou definované vzhledem ke stejným mírám a tudíž se mohou porovnávat.  \par 
Předpokládejme tedy, že $x$, $x^{,}$ mají dimenzi $n$ respektive $n^{,}$ a pomocné náhodné veličiny $u$, $u^{,}$ mají dimenzi $r$ respektive $r^{,}$. Nyní mějme deterministické, diferencovatelné, prosté funkce $h_p: \mathbb{R}^n \times \mathbb{R}^r: \rightarrow \mathbb{R}^{n^{,}} \times \mathbb{R}^{r^{,}}$ a $h^{,}_p: \mathbb{R}^{n^{,}} \times \mathbb{R}^{r^{,}} : \rightarrow \mathbb{R}^n \times \mathbb{R}^r$, takové, že $h_p(x, u) = (x^{,}, u^{,})$ a $h_p^{,}(x^{,}, u^{,}) = (x, u)$, kde $p$ je index přeskoku z konečné množiny $P$. Pro tyto funkce musí platit, aby $n+r=n^{,}+r^{,}$, kdyby toto neplatilo, nebudou obě transformace diferencovatelné \cite{green2009reversible}. Náhodné veličiny $u, u^{,}$ mají pravděpodobnostní distribuci $g_p(u)$, respektive $g^{,}_p(u)$ a jejich konkrétní tvar záleží na daném problému. Samotná pravděpodobnost toho, že se ve stavu $x$ pokusíme o přechod $p$ je dána pravděpodobnostní hustotou $j_p(x)$ respektive $j^{,}_p(x)$, pro konkrétní problém je možné nebo i výhodné, aby množina přechodů, o které se algoritmus pokusí v daném stavu $x$, byla podmnožinou $P$.    Samotná přechodová pravděpodobnost je dána:
\begin{align}\label{rjmcmc_probability}
A_p(x, x^{,}) = min\{1, \frac{\pi(x^{,})j_p(x^{,})g^{,}_p(u^{,})}{\pi(x)j_p(x)g_p(u)}
\begin{vmatrix} \frac{\partial(\theta^{,}_{k^{,}}, u^{,})}{(\theta_k, u)} \end{vmatrix}\}.
\end{align}
...
Model bude vždy stejný a to sice model zadefinovaný zde \ref{supermodel}. Omezíme se na model s maximálně 30 zlomy, je to hlavně technické omezení, později uvidíme, že se reálně k 30 zlomům nepřiblížíme. Možností jak volit transformační funkce mezi dvěma modely je obecně nekonečno. Transformaci pro přechod mezi modelem s $m$ zlomy a $m+1$ zlomy volíme takto:
\begin{align*}
	...
	n \sim N(0, 3). \\
\end{align*}
Náhodná veličina $u$ slouží k určení, kde mezi počátečním a koncovým bodem by se měl nacházet nový bod. Náhodná veličina $n$ určuje to, o kolik výš nebo níž by měl nový bod být oproti původní přímce. V této transformaci je vidět, že spojitou po částech lineární funkci lámeme mezi body 1 a 2,  
